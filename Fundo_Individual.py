...
import importlib
import modulos.risk_engine
importlib.reload(modulos.risk_engine)
...
def carregar_sistema_v2():
    print("DEBUG: Iniciando carregar_sistema_v2 (Setup Silencioso)")
    
    db_path = 'risk_system.db'
    zip_path = 'risk_system.zip'
    excel_path = 'Quant_Fundos.xlsm'
    
    # 1. Verifica se o DB já existe
    if not os.path.exists(db_path):
        print("DEBUG: Banco de dados não encontrado. Iniciando recuperação...")
        
        db_criado = False
        
        # TENTATIVA 1: Descompactar ZIP
        if os.path.exists(zip_path):
            try:
                print(f"DEBUG: Tentando descompactar {zip_path}")
                with zipfile.ZipFile(zip_path, 'r') as zip_ref:
                    zip_ref.extractall('.')
                if os.path.exists(db_path):
                    print("DEBUG: Banco descompactado com sucesso.")
                    db_criado = True
            except Exception as e:
                print(f"ERRO DECOMPRESSAO: {e}")
        
        # TENTATIVA 2: Criar do Excel (Fallback)
        if not db_criado and os.path.exists(excel_path):
            try:
                print("DEBUG: Invocando criação via Excel...")
                db = RiskDatabase(db_path)
                db.create_schema() # GARANTIA DE ESTRUTURA
                db.importar_excel_principal(excel_path)
                print("DEBUG: Banco criado via Excel.")
                db_criado = True
            except Exception as e:
                print(f"ERRO IMPORTACAO EXCEL: {e}")
                st.error("Erro crítico ao inicializar dados. Consulte o administrador.")
                st.stop()
        
        if not db_criado:
             st.error("Erro fatal: Arquivos de dados (DB, ZIP ou Excel) não encontrados.")
             st.stop()
...
with st.sidebar:
    ...
    if 'fundo_selecionado' not in st.session_state:
        fundos_disp_init = loader.listar_ativos('Fundo')
        st.session_state.fundo_selecionado = fundos_disp_init[0] if fundos_disp_init else None
    if 'fundo_filtro_classe' not in st.session_state:
        st.session_state.fundo_filtro_classe = "Todas"
    if 'fundo_filtro_subclasse' not in st.session_state:
        st.session_state.fundo_filtro_subclasse = "Todas"
    if 'fundo_filtro_grupo' not in st.session_state:
        st.session_state.fundo_filtro_grupo = "Todos"
    if 'fundo_periodo' not in st.session_state:
        st.session_state.fundo_periodo = "12 Meses"
    ...
    try:
        index_fundo = fundos_disp.index(st.session_state.fundo_selecionado)
    except (ValueError, AttributeError):
        index_fundo = 0
        st.session_state.fundo_selecionado = fundos_disp[0] if fundos_disp else None
    
    fundo_selecionado = st.selectbox(
        "Ativo Principal", 
        options=fundos_disp,
        index=index_fundo,
        key='selectbox_fundo_principal'
    )
    st.session_state.fundo_selecionado = fundo_selecionado
    
    benchs_possiveis = [b for b in benchs_disp if b != fundo_selecionado]
    default_benchs = [b for b in ["Ibovespa", "IBOV", "CDI", "PTAXV"] if b in benchs_possiveis]
    ...
    with st.expander("Filtros Avançados de Busca"):
        ...
        try:
            index_classe = classes_disp.index(st.session_state.fundo_filtro_classe)
        except ValueError:
            index_classe = 0
            st.session_state.fundo_filtro_classe = "Todas"
        ...
        filtro_classe = st.selectbox(
            ...
            index=index_classe, 
            key="filt_classe_comp"
        )
        st.session_state.fundo_filtro_classe = filtro_classe
        ...
        try:
            index_subclasse = subclasses_disp.index(st.session_state.fundo_filtro_subclasse)
        except ValueError:
            index_subclasse = 0
            st.session_state.fundo_filtro_subclasse = "Todas"
        ...
        st.session_state.fundo_filtro_subclasse = filtro_subclasse
        ...
        try:
            index_grupo = grupos_disp.index(st.session_state.fundo_filtro_grupo)
        except ValueError:
            index_grupo = 0
            st.session_state.fundo_filtro_grupo = "Todos"
        ...
        st.session_state.fundo_filtro_grupo = filtro_grupo
    ...
    opcoes_periodo = ["Mês Atual", "3 Meses", "6 Meses", "12 Meses", "24 Meses", "36 Meses", "48 Meses", "60 Meses", "Desde o Início"]
    try:
        index_periodo = opcoes_periodo.index(st.session_state.fundo_periodo)
    except ValueError:
        index_periodo = 3  # Default 12m
        st.session_state.fundo_periodo = "12 Meses"
    ...
    periodo_opcao = st.radio(
        ...
        index=index_periodo,
        key='radio_periodo_fundo'
    )
    st.session_state.fundo_periodo = periodo_opcao
...
# Definir mapa_dias fora do bloco para uso posterior
mapa_dias = {
    "3 Meses": 63, "6 Meses": 126, "12 Meses": 252, 
    "24 Meses": 504, "36 Meses": 756, "48 Meses": 1008, 
    "60 Meses": 1260, "Desde o Início": len(df_dados_full)
}

if periodo_opcao == "Mês Atual":
    # Filtrar apenas o mês da última data disponível
    ultima_data = df_dados_full.index[-1]
    ano_mes = ultima_data.year, ultima_data.month
    df_dados = df_dados_full[(df_dados_full.index.year == ano_mes[0]) & (df_dados_full.index.month == ano_mes[1])].copy()
else:
...
# --- CÁLCULO DE MÉTRICAS DO CABEÇALHO ---
# 1. Retorno Anualizado desde o Início
ret_anual_inicio = 0.0
dias_full = 0
if not s_fundo_full.empty:
    ret_total_full = (1 + s_fundo_full).prod() - 1
    dias_full = len(s_fundo_full)
    if dias_full > 0:
        ret_anual_inicio = (1 + ret_total_full) ** (252 / dias_full) - 1

# 2. Retorno Relativo (Benchmark)
# Identificar classe e benchmark referência
info_fundo = loader.mapa_nomes.get(fundo_selecionado, {})
classe_fundo = info_fundo.get('classe', '')

# LOGICA CORRIGIDA: No banco de dados, Fundo de Ações está classificado como 'RV'
is_equity = False
if str(classe_fundo).upper() == 'RV':
    is_equity = True

# Definir nome do benchmark de referência
nome_bench_ref = 'CDI'
if is_equity:
    # Tenta encontrar variante do Ibovespa
    for cand in ['Ibovespa', 'IBOV', 'IBOVESPA']:
        if cand in loader.mapa_nomes:
            nome_bench_ref = cand
            break

# Buscar dados do benchmark apenas para o cálculo
df_ref = loader.buscar_dados([nome_bench_ref])

val_relativo = 0.0
str_val_relativo = "-"
label_relativo = f"vs {nome_bench_ref}"

if not df_ref.empty and nome_bench_ref in df_ref.columns:
    # Alinhar com o histórico completo do fundo
    s_ref = df_ref[nome_bench_ref].reindex(s_fundo_full.index).fillna(0)
    
    if not s_ref.empty:
        ret_total_ref_full = (1 + s_ref).prod() - 1
        
        if is_equity:
            # Para ações: Alpha Anualizado vs Ibovespa
            dias_ref = len(s_ref) # Mesmos dias do fundo pois foi reindexado
            if dias_ref > 0:
                ret_anual_ref = (1 + ret_total_ref_full) ** (252 / dias_ref) - 1
                val_relativo = ret_anual_inicio - ret_anual_ref
                label_relativo = f"Alpha Anualizado ({nome_bench_ref})"
                str_val_relativo = f"{val_relativo:+.2%}"
        else:
            # Para outros: % do CDI (Acumulado desde o início)
            if ret_total_ref_full != 0:
                val_relativo = ret_total_full / ret_total_ref_full
                label_relativo = f"% {nome_bench_ref} (Início)"
                str_val_relativo = f"{val_relativo:.0%}"

# Cabeçalho do Fundo
col_header_1, col_h2, col_h3, col_h4 = st.columns([2, 1, 1, 1])
with col_header_1:
    st.markdown(f"### {fundo_selecionado}")
    st.markdown(f"**Início:** {first_idx.strftime('%d/%m/%Y')}")

with col_h2:
    # Retorno acumulado SEMPRE DO INÍCIO
    # ret_total_full foi calculado nas linhas anteriores (bloco CÁLCULO DE MÉTRICAS DO CABEÇALHO)
    if 'ret_total_full' in locals():
         st.metric("Retorno Acumulado (Início)", f"{ret_total_full:.2%}")
    else:
         st.metric("Retorno Acumulado (Início)", "-")

with col_h3:
    st.metric("Retorno Anualizado (Início)", f"{ret_anual_inicio:.2%}")

with col_h4:
    st.metric(label_relativo, str_val_relativo)
...
with tab_perf:
    ...
    # KPIs Principais
    # KPIs Principais (Baseados no Período Selecionado: df_dados)
    
    # 1. Retorno do Período
    ret_periodo_sel = (1 + s_fundo).prod() - 1
    
    # 2. Volatilidade Anualizada do Período
    vol_periodo = s_fundo.std() * (252 ** 0.5)
    
    # 3. Sharpe do Período
    try:
        # Tenta reutilizar ou carregar CDI para cálculo do excesso de retorno
        if 'CDI' in df_dados.columns:
            s_cdi_slice = df_dados['CDI']
        else:
            # Se não estiver no slice atual, busca
            df_cdi_tmp = loader.buscar_dados(['CDI'])
            s_cdi_slice = df_cdi_tmp['CDI'].reindex(s_fundo.index).fillna(0)

        ret_excesso = s_fundo - s_cdi_slice
        sharpe_periodo = (ret_excesso.mean() * 252) / (s_fundo.std() * (252 ** 0.5))
    except:
        sharpe_periodo = 0.0
        
    # 4. IPCA + (Spread sobre IPCA no período)
    # Busca IPCA apenas se necessário (Nome 'IPCA' confirmado via debug)
    nome_ipca = "IPCA"
    
    spread_ipca_anual = 0.0
    label_ipca = "IPCA + (N/A)"
    
    try:
        df_ipca = loader.buscar_dados([nome_ipca])
        
        if not df_ipca.empty and nome_ipca in df_ipca.columns:
            # Reindexar para o período do fundo (Preencher NaNs com 0 assumindo dias sem cota = sem inflação computada ou irrelevante)
            s_ipca_periodo = df_ipca[nome_ipca].reindex(s_fundo.index).fillna(0)
            
            # Calcular retorno total do IPCA no período
            ret_ipca_total = (1 + s_ipca_periodo).prod() - 1
            
            # Calcular Spread Total: (1 + R_fundo) = (1 + R_ipca) * (1 + Spread)
            if (1 + ret_ipca_total) != 0:
                spread_total = (1 + ret_periodo_sel) / (1 + ret_ipca_total) - 1
                
                # Anualizar o spread para apresentar como "Taxa IPCA+"
                dias_uteis = len(s_fundo)
                if dias_uteis > 20: 
                    spread_ipca_anual = (1 + spread_total) ** (252 / dias_uteis) - 1
                    label_ipca = "Equivalente IPCA +"
                else:
                    spread_ipca_anual = spread_total
                    label_ipca = "Spread IPCA (Período)"
    except Exception as e:
        print(f"Erro IPCA+: {e}")
        label_ipca = "Erro IPCA"

    label_p = periodo_opcao.replace("Meses", "m").replace("Desde o Início", "Início")
    
    kp1, kp2, kp3, kp4 = st.columns(4)
    kp1.metric(f"Retorno ({label_p})", f"{ret_periodo_sel:.2%}")
    kp2.metric(f"Volatilidade ({label_p})", f"{vol_periodo:.2%}")
    kp3.metric(f"Sharpe ({label_p})", f"{sharpe_periodo:.2f}")
    kp4.metric(label_ipca, f"{spread_ipca_anual:.2%}")
    ...
    # Benchmarks como linha pontilhada (dash) para diferenciação
    if benchmarks_selecionados:
        for b_mark in benchmarks_selecionados:
             fig_perf.update_traces(selector=dict(name=b_mark), line=dict(dash='dash', width=2))
    ...
    with c_roll_opts:
        ...
        # Filtra "Mês Atual" e "Desde o Início" das opções globais pois não fazem sentido para janela fixa
        opcoes_janela_movel = [opt for opt in mapa_dias.keys() if opt not in ["Desde o Início"]]
...
with tab_style:
    ...
    if len(benchmarks_selecionados) >= 1:
        ...
        with col_ctrl1:
            st.write("") # Spacer
        with col_ctrl2:
            permitir_alavancagem = st.toggle("Permitir Alavancagem (Soma livre)", value=False, help="Se ativado, a soma dos pesos pode ultrapassar 100%, permitindo identificar alavancagem nos fatores.")
        
        # Executar Engine RBSA
        # constrained=True significa Soma <= 1 (Sem alavancagem explícita na soma). 
        # Se permitir_alavancagem=True, então constrained=False.
        coefs, r2, fitted = engine.analise_estilo_rbsa(s_fundo, df_indicies, constrained=not permitir_alavancagem)
        ...
        if coefs:
            ...
            with col_s1:
                ...
                # Se alavancagem permitida, mostrar Exposição Total
                soma_pesos = sum([v for k,v in coefs.items() if k not in ['const', 'Alpha', 'Residual_Cash', 'R2']])
                if permitir_alavancagem:
                    st.metric("Exposição Total (Bruta)", f"{soma_pesos:.0%}", delta="Alavancagem" if soma_pesos > 1 else None)
                else:
                    st.metric("Exposição Total", f"{soma_pesos:.0%}")
            ...
            with col_s2:
                ...
                # Remover constante/Alpha e manter o resto (incluindo Residual_Cash se existir)
            ...
            # NOTA: O rolling ainda não tem o toggle de alavancagem implementado no engine, 
            # ou precisaríamos atualizar calcular_rolling_rbsa também.
            # Vamos manter rolling como constrained default por enquanto para não pesar, 
            # ou idealmente, passar a restrição lá também.
            # O ideal é atualizar calcular_rolling_rbsa no engine se quisermos consistência.
            # Mas como o usuário perguntou "como mostra o beta", o chart de barras atual já mostra.
            # Manterei o rolling como 'padrão' (constrained) para comparar a evolução estrutural vs alavancagem pontual.
            # (Ou se quiser, avisar que o rolling é sempre constrained).
            ...
            # --- Cards de Alpha ---
            # Recalcular Sintético Passivo (Apenas exposição aos Betas, SEM o Alpha estatístico)
            # O 'fitted' original incluía o Alpha, o que forçava a média dos resíduos a zero.
            
            # 1. Recuperar dados alinhados (Fundo e Benchs)
            # O fitted.index representa as datas que foram efetivamente usadas na regressão (interseção)
            idxs_validos = fitted.index
            fund_aligned = s_fundo.loc[idxs_validos]
            benchs_aligned = df_indicies.loc[idxs_validos]
            
            # 2. Reconstruir Sintético Passivo (Beta Portfolio)
            sintetico_passivo = pd.Series(0.0, index=idxs_validos)
            
            for fator, peso in coefs.items():
                if fator not in ['const', 'Alpha', 'Residual_Cash', 'R2'] and fator in benchs_aligned.columns:
                    sintetico_passivo += benchs_aligned[fator] * peso
            
            # 3. Calcular Excesso de Retorno (Alpha Diário + Ruído)
            excess_return_series = fund_aligned - sintetico_passivo
            
            # Cálculo das Métricas Anualizadas
            alpha_anual = excess_return_series.mean() * 252
            tracking_error = excess_return_series.std() * (252**0.5)
            
            if tracking_error > 0:
                info_ratio = alpha_anual / tracking_error
            else:
                info_ratio = 0.0
            ...
            # Layout Cards
            c_alk1, c_alk2, c_alk3, c_alk4 = st.columns(4)
            
            with c_alk1:
                # Alpha
                delta_lbl = "Positivo" if alpha_anual > 0 else "Negativo"
                st.metric(
                    "Alpha a.a.", 
                    f"{alpha_anual:+.2%}", 
                    delta=delta_lbl,
            ...
            with c_alk2:
                # Tracking Error
                label_te = "Baixo" if tracking_error < 0.02 else ("Moderado" if tracking_error < 0.04 else "Alto")
                st.metric(
                    "Tracking Error",
                    f"{tracking_error:.2%}",
                    delta=label_te,
            ...
            with c_alk3:
                # R2
                label_r2 = "Alta" if r2 > 0.7 else ("Média" if r2 > 0.5 else "Baixa")
                st.metric(
                    "R² (Explicação)",
                    f"{r2:.2%}",
                    delta=label_r2,
            ...
            with c_alk4:
                ...
                st.metric(
                    ...
                    f"{info_ratio:.2f}",
                    delta=lbl_ir,
                    delta_color=color_ir
                )

            st.caption("O 'Sintético' aqui representa o retorno passivo dos índices ponderados (Betas). A diferença acumulada representa o valor agregado (Alpha) pelo gestor.")
            
            # Gráfico de Linha (Agora usando o Sintético Passivo para evidenciar o Alpha)
            wealth_fund = (1 + fund_aligned).cumprod() * 100
            wealth_synth = (1 + sintetico_passivo).cumprod() * 100
            
            df_alpha_plot = pd.DataFrame({
                'Fundo Real': wealth_fund,
                'Sintético (Passivo)': wealth_synth
            })
            
            fig_alpha = px.line(df_alpha_plot, color_discrete_sequence=[paleta_cores[0], "#7f7f7f"]) # Azul e Cinza
            
            # Adicionar área preenchida entre as linhas para destacar o Alpha?
            # Por enquanto manter linha limpa, mas destacando 'Sintético' como cinza
            
            fig_alpha.update_traces(line=dict(width=2))
            fig_alpha.update_traces(selector=dict(name='Fundo Real'), line=dict(width=3.5))
            fig_alpha.update_traces(selector=dict(name='Sintético (Passivo)'), line=dict(dash='dash'))
