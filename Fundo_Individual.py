...
def carregar_sistema_v2():
    print("DEBUG: Iniciando carregar_sistema_v2")
    
    db_path = 'risk_system.db'
    zip_path = 'risk_system.zip'
    
    # 1. Verifica se o DB existe
    if not os.path.exists(db_path):
        # 2. Se não existir, tenta descompactar o ZIP
        if os.path.exists(zip_path):
            st.warning("Banco de dados compactado detectado. Iniciando descompactação...")
            try:
                with zipfile.ZipFile(zip_path, 'r') as zip_ref:
                    zip_ref.extractall('.')
                st.success("Banco de dados descompactado com sucesso!")
            except Exception as e:
                st.error(f"Erro ao descompactar banco de dados: {e}")
                st.stop()
        else:
            # Opção de fallback ou erro se nem o zip existir (e não quisermos recriar do zero)
            # Como você preferiu compressão, vamos assumir que o zip deve estar lá.
            # Se quiser manter a lógica de recriar do Excel como ultimo recurso, podemos manter.
            pass
