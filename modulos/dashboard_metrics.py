...
class DashboardMetrics:
    ...
    def calcular_retorno_mensal(self, series):
        """
        Calcula o retorno do mês mais recente baseado na última data disponível.
        Por exemplo, se a última data é 26/12/2025, calcula o retorno de dezembro/2025.
        Retorna em percentual (ex: 2.5 para 2.5%).
        """
        if series.empty:
            return np.nan
        ...
        ultima_data = series.index[-1]
        ano_mes = ultima_data.year, ultima_data.month
        
        # Filtrar apenas o mês da última data
        series_mes = series[(series.index.year == ano_mes[0]) & (series.index.month == ano_mes[1])]
        
        if series_mes.empty:
            return np.nan
            
        # Retorno composto do mês
        return ((1 + series_mes).prod() - 1) * 100.0
