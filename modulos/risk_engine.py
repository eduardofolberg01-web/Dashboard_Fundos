...
class RiskEngine:
    ...
    def _executar_otimizacao_rbsa(self, y, X):
        """
        ...
        # MUDANÇA: Soma deve ser MENOR ou IGUAL a 1 (1 - soma >= 0)
        # Isso permite identificar Caixa ou exposição não mapeada.
        constraints = ({'type': 'ineq', 'fun': lambda w: 1 - np.sum(w)})
        
        # Limites (Bounds): 0 <= w <= 1 (No-Shorting)
        bounds = tuple((0, 1) for _ in range(n_factors))
        
        # Chute inicial: divisão igualitária, mas ajustada para ser < 1
    ...
    def analise_estilo_rbsa(self, fundo_series, benchmarks_df):
        """
        ...
        weights, alpha, r2 = self._executar_otimizacao_rbsa(y, X)
