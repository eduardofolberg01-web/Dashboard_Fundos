...
st.set_page_config(
    page_title="Otimizador Markowitz", 
    layout="wide",
...
with st.sidebar:
    ...
    try:
        ...
        # Calcula retorno médio diário do CDI
        ret_medio_diario_cdi = df_cdi_slice.mean()
        
        # Anualiza: (1 + ret_diario)^252 - 1
        n_dias_uteis = len(df_cdi_slice)
        if n_dias_uteis > 0:
            rf_rate_calc = (1 + ret_medio_diario_cdi) ** 252 - 1
        else:
    ...
    st.metric(
        "Taxa Livre de Risco (CDI Anualizado)", 
        f"{rf_rate_calc:.2%}", 
        help=f"Calculado com base no retorno médio diário do CDI no período selecionado ({janela}), anualizado para 252 dias úteis. Esta taxa é utilizada no cálculo do Sharpe Ratio: (Retorno - CDI) / Volatilidade"
    )
    ...
    with st.expander("Limites de Alocação (Mínimo e Máximo)"):
        ...
        # Inicializar valores no session_state ANTES de criar os widgets
        for ativo in ativos_sel:
            if f"min_{ativo}" not in st.session_state:
                st.session_state[f"min_{ativo}"] = 0.0
            if f"max_{ativo}" not in st.session_state:
                st.session_state[f"max_{ativo}"] = 100.0
        ...
        for ativo in ativos_sel:
            ...
            with c_min:
                # Não define 'value' - usa apenas o session_state via 'key'
...
cache_key = f"{sorted(ativos_sel)}_{janela}_{definir_target}_{target_vol if definir_target else 'none'}"
...
# Verifica se os parâmetros mudaram (invalida cache)
if 'cache_key' in st.session_state and st.session_state['cache_key'] != cache_key:
    if 'opt_results' in st.session_state:
        del st.session_state['opt_results']
    st.session_state['cache_key'] = cache_key
...
if rodar or 'opt_results' not in st.session_state:
    ...
    if not first_valid_indices.dropna().empty:
        ...
        st.info(f"Informação sobre o Período: O histórico comum dos ativos selecionados começa em {max_start_date.strftime('%d/%m/%Y')} (limitado pelo ativo {ativo_limitante}). A otimização utiliza apenas o período em que todos os ativos possuem dados (interseção).")

    # Remove linhas com dados faltantes (usa apenas período comum)
    df_ret = df_raw.dropna().copy()
    
    if df_ret.empty:
        st.error("Não há período comum com dados para todos os ativos selecionados.")
        st.stop()
    
    # Detecta e corrige escala dos dados
    # Verifica se os dados estão em percentual (ex: 1.0 para 1%) ou decimal (ex: 0.01 para 1%)
    # Heurística: se a média absoluta for > 0.1, provavelmente está em percentual
    if df_ret.abs().mean().mean() > 0.1:
        st.warning("⚠️ Detectado dados em percentual inteiro (ex: 1.0 para 1%). Convertendo para decimal (ex: 0.01).")
        df_ret = df_ret / 100.0
    ...
    # Verifica se há dados suficientes para a janela solicitada
    if len(df_ret) < dias:
        st.warning(f"⚠️ Atenção: Janela solicitada ({janela} = {dias} dias úteis) maior que o histórico disponível ({len(df_ret)} dias). Usando todo o histórico disponível.")
        dias = len(df_ret)
        
    df_ret = df_ret.tail(dias)
    
    # Validação final: verificar se há dados mínimos
    if len(df_ret) < 60:  # Mínimo de ~3 meses
        st.error(f"Dados insuficientes para análise confiável. Apenas {len(df_ret)} dias úteis disponíveis (mínimo recomendado: 60).")
        st.stop()
    
    # Informação sobre o período efetivamente utilizado
    st.success(f"✅ **Período de Análise**: {df_ret.index[0].strftime('%d/%m/%Y')} a {df_ret.index[-1].strftime('%d/%m/%Y')} ({len(df_ret)} dias úteis)")
    ...
    results_cache = {
        'df_ret': df_ret,
        'bounds': bounds,
        'rf_rate': rf_rate  # Salva a taxa livre de risco para exibição posterior
    }
...
if 'opt_results' in st.session_state:
    ...
    if cache['mode'] == 'target':
        result = cache['result']
        if result:
            # Recupera a taxa livre de risco do cache
            rf_cached = cache.get('rf_rate', 0.0)
            ...
            c3.metric(
                "Sharpe Ratio", 
                f"{result['Sharpe']:.2f}",
                help=f"Calculado como (Retorno - CDI) / Volatilidade. CDI utilizado: {rf_cached:.2%} a.a."
            )
            c4.metric("CDI (Taxa Livre de Risco)", f"{rf_cached:.2%}")
        ...
        # Exibe informação sobre o CDI utilizado
        st.info(f"Sharpe Ratio calculado como (Retorno - CDI) / Volatilidade, onde CDI = {rf_cached:.2%} a.a.")
