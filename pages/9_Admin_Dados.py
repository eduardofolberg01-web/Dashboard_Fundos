...
import os
import time
import zipfile # Adicionar import no topo
        ...
        if st.button("🔄 Sincronizar Agora", type="primary"):
            with st.status("Processando atualização...", expanded=True) as status:
                ...
                try:
                    ...
                    st.write("Compactando novo banco de dados (risk_system.zip)...")
                    with zipfile.ZipFile(os.path.join(root_dir, 'risk_system.zip'), 'w', zipfile.ZIP_DEFLATED) as zipf:
                        zipf.write(os.path.join(root_dir, 'risk_system.db'), arcname='risk_system.db')
                    
                    st.write("✅ Dados importados e ZIP atualizado!")
                    status.update(label="Atualização Completa!", state="complete", expanded=False)
                    st.success("Base atualizada! O arquivo 'risk_system.zip' foi recriado. Lembre-se de fazer o COMMIT dele no Git.")
                    time.sleep(3) # Tempo pra ler antes de recarregar
                    st.rerun()
